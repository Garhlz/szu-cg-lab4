...
void TriMesh::storeFacesPoints()
{
	...
	{
		// 获取当前面片的 顶点、颜色、法线、纹理 的索引
		// 注意：每个 face 有 x, y, z 三个顶点
		...
		// 2. 颜色 (Color) - 使用 color_index
		// 如果 obj 文件没颜色，通常我们在 readObj 里会用临时数据填充 vertex_colors
		...
		// 4. 纹理坐标 (Texture) - 使用 texture_index
		// 实验文档要求 vertex_textures 存储 UV 坐标数据
...
void TriMesh::readObj(const std::string &filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		if (type == "v")
		{
			...
			// 实验要求：因obj无法存储颜色，可以使用法线(或位置)作为颜色数值初始化
		...
		else if (type == "f")
		{
			...
			for (int i = 0; i < 3; i++)
			{
				sin >> v_str; // 读取如 "2/1/1" 的字符串

				// 将 / 替换为空格以便 stringstream 解析
				for (char &c : v_str)
					if (c == '/')
						c = ' ';

				std::istringstream v_sin(v_str);
				v_sin >> v_idx[i] >> vt_idx[i] >> vn_idx[i];

				// OBJ索引从1开始，需要减1转为0起始的数组下标
				v_idx[i]--;
				vt_idx[i]--;
				vn_idx[i]--;
			}
			...
			// 存储索引
			faces.push_back(vec3i(v_idx[0], v_idx[1], v_idx[2]));
			texture_index.push_back(vec3i(vt_idx[0], vt_idx[1], vt_idx[2]));
			normal_index.push_back(vec3i(vn_idx[0], vn_idx[1], vn_idx[2]));

			// 颜色索引通常跟随顶点索引
			color_index.push_back(vec3i(v_idx[0], v_idx[1], v_idx[2]));
		}
	}
	storeFacesPoints(); // 这一步至关重要，将解析的数据展平
}
// Light
