...
void init()
{
	...
	// 设置桌子纹理和变换
	table->setTranslation(glm::vec3(0.0, -0.5, 0.0)); // 往下移一点
	table->setRotation(glm::vec3(0.0, 0.0, 0.0));
	table->setScale(glm::vec3(2.0, 2.0, 2.0)); // 放大一点
	...
	wawa->readObj("./assets/wawa.obj"); // 确保文件名正确

	// 设置娃娃变换，放在桌子上
	wawa->setTranslation(glm::vec3(0.0, 0.4, 0.0));
	wawa->setRotation(glm::vec3(0.0, 0.0, 0.0));
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	{
		// 修改这里：允许 PRESS 和 REPEAT 两种状态通过
		// 这样你按住 'u', 'i', 'o' 时，画面会连续变化，体验更流畅
